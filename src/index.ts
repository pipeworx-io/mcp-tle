...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'TLE');
}
...
async function getTle(noradId: number): Promise<unknown> {
  const res = await pwFetch(`${BASE_URL}/${noradId}`);
  if (!res.ok) throw await httpError(res, 'TLE API error');
...
async function listRecent(limit: number): Promise<unknown> {
  const params = new URLSearchParams({
    // `date` is not a valid sort for this API — it 400s with "[sort] The value you
    // selected is not a valid choice", which is why this tool has never once
    // succeeded. The API accepts only `id`, `name` and `popularity`; there is no
    // epoch-date ordering to be had. `id` descending is the honest substitute:
    // NORAD catalog numbers are assigned in roughly launch order, so newest-id-first
    // approximates newest-object-first. The description says so rather than
    // continuing to promise a sort the upstream cannot do.
    sort: 'id',
    'sort-dir': 'desc',
    'page-size': String(limit),
  });
